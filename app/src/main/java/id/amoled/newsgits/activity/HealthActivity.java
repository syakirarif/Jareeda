...
public class HealthActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listBusiness = findViewById(R.id.list_health); //GANTI INI
        listBusiness.setHasFixedSize(true);
        RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(this);
        listBusiness.setLayoutManager(layoutManager);

        swipeLayout = findViewById(R.id.layout_swipe_health); //GANTI INI
    ...
    private void loadArticleSource(boolean isRefreshed) {
        ...
        if (!isRefreshed) {
            ...
            String cache = Paper.book().read("img_health"); //GANTI INI
            ...
            if (cache != null && !cache.isEmpty() && !cache.equals("null")) {
                ...
                mService.getHealth().enqueue(new Callback<WebSite>() { //GANTI INI
                    @Override
                    public void onResponse(@NonNull Call<WebSite> call, @NonNull Response<WebSite> response) {
                        ...
                        Paper.book().write("img_health", new Gson().toJson(response.body())); //GANTI INI
                        swipeLayout.setRefreshing(false);
                    }

                    @Override
                    public void onFailure(@NonNull Call<WebSite> call, @NonNull Throwable t) {
                        Toast.makeText(HealthActivity.this, t.getMessage(), Toast.LENGTH_SHORT).show();
                        swipeLayout.setRefreshing(false);
