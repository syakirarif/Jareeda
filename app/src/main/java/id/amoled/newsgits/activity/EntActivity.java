...
public class EntActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        listBusiness = findViewById(R.id.list_ent); //GANTI INI
        listBusiness.setHasFixedSize(true);
        RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(this);
        listBusiness.setLayoutManager(layoutManager);

        swipeLayout = findViewById(R.id.layout_swipe_ent); //GANTI INI
    ...
    private void loadArticleSource(boolean isRefreshed) {
        ...
        if (!isRefreshed) {
            ...
            String cache = Paper.book().read("ent"); //GANTI INI
            //String cache2 = Paper.book().read("sumber");
            if (cache != null && !cache.isEmpty() && !cache.equals("null")) {
                WebSite webSite = new Gson().fromJson(cache, WebSite.class);
                //Articles sources = new Gson().fromJson(cache2, Articles.class);
                adapter = new AdapterNews(getBaseContext(), webSite);
                adapter.notifyDataSetChanged();
                listBusiness.setAdapter(adapter);
                swipeLayout.setRefreshing(false);
            } else {
                mService.getEntertainment().enqueue(new Callback<WebSite>() { //GANTI INI
                    @Override
                    public void onResponse(@NonNull Call<WebSite> call, @NonNull Response<WebSite> response) {
                        ...
                        Paper.book().write("ent", new Gson().toJson(response.body())); //GANTI INI
                        swipeLayout.setRefreshing(false);
                    }

                    @Override
                    public void onFailure(@NonNull Call<WebSite> call, @NonNull Throwable t) {
                        Toast.makeText(EntActivity.this, t.getMessage(), Toast.LENGTH_SHORT).show();
                        swipeLayout.setRefreshing(false);
                    }
                });
            }
