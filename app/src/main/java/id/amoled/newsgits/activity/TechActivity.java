...
public class TechActivity extends AppCompatActivity {
    ...
    private void loadArticleSource(boolean isRefreshed) {
        ...
        if (!isRefreshed) {
            ...
            String cache = Paper.book().read("img_tech"); //GANTI INI
            ...
            if (cache != null && !cache.isEmpty() && !cache.equals("null")) {
                ...
                mService.getTech().enqueue(new Callback<WebSite>() { //GANTI INI
                    @Override
                    public void onResponse(@NonNull Call<WebSite> call, @NonNull Response<WebSite> response) {
                        ...
                        Paper.book().write("img_tech", new Gson().toJson(response.body())); //GANTI INI
                        swipeLayout.setRefreshing(false);
                    }

                    @Override
                    public void onFailure(@NonNull Call<WebSite> call, @NonNull Throwable t) {
                        Toast.makeText(TechActivity.this, t.getMessage(), Toast.LENGTH_SHORT).show();
                        swipeLayout.setRefreshing(false);
                    }
                });
            }
